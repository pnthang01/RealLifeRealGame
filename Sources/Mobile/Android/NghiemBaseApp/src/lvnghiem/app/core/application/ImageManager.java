...
import lvnghiem.app.core.utils.NumberUtils;
...
public class ImageManager extends BaseController
{
	...
	public boolean loadDrawable(View imageView, String link, int viewWidth, int viewHeight, boolean useBackgroundTransitionAnimation)
	{
		...
		{
			// At least link = http://
			LOG.debug("Load drawble from link --- Wrong URL!!! URL = " + link);
			return true;
		}
	...
	public void saveToSdcard(String link, Bitmap bitmap, boolean useHighQuality)
	{
		if (TextUtils.isEmpty(link))
		{
			LOG.debug("Save image to SDCard: cancel because URL is null");
			return;
		}
		if (bitmap == null)
		{
			LOG.debug("Save image to SDCard: cancel because bitmap is null (link: " + link + ")");
			return;
		}
		if (!DeviceManager.isExternalStorageSupportWrite())
		{
			LOG.debug("Save image to SDCard: cancel, there is no SDCard or it is read only");
			return;
		}
		...
		if (!file.exists())
		{
			...
			{
				LOG.debug("Save image to SDCard: error =  " + e.getMessage());
				return;
			}
		}
		else
	...
	public void deleleFromSdcard(String linkWithColor)
	{
		...
		{
			LOG.debug("Delete image from SDCard: cancel - Null link");
			return;
		}
		if (!DeviceManager.isExternalStorageSupportWrite())
		{
			LOG.debug("Delete image from SDCard: cancel - There is no SDCard or it is read only");
			return;
		}
		...
		if (linkWithColor.contains(COLOR_PREFIX))
		{
			...
			{
				// At least link = http://
				LOG.debug("Delete image from SDCard: cancel - wrong link: " + link);
				return;
			}
		}
		else
		...
		{
			LOG.debug("Delete image from SDCard: link is not cached yet!!! (link: " + link + ")");
		}
	}
	...
	public Bitmap loadFromSdcard(String link, int viewWidth, int viewHeight)
	{
		if (TextUtils.isEmpty(link))
		{
			LOG.debug("Load image from SDCard: cancel - Null link");
			return null;
		}
		if (!DeviceManager.isExternalStorageSupportWrite())
		{
			LOG.debug("Load image from SDCard: cancel - There is no SDCard or it is read only");
			return null;
		}
		...
		if (!file.exists())
		{
			LOG.debug("Load image from SDCard: cancel - This was not saved to SDCard yet");
			return null;
		}
