...
public class WebManager extends BaseController
{
    private static final String WEB_CONTENT = "<html><head>"
            + "<style type='text/css'>"
            + "body {font-size: %dpx; text-align: left; margin: 0px 0px 0px 0px;}"
            + "</style></head>" + "<body %s>%s</body>" + "</html>";

    private static final String WEB_CONTENT_FONT_FACE_APPLIED = "<html><head>"
            + "<style type='text/css'>"
            + "@font-face {font-family: 'custom_font'; src: url('%s')} body {font-family: 'custom_font'; "
            + "font-size: %dpx;text-align: left; margin: 0px 0px 0px 0px;}</style></head>"
            + "<body %s>%s</body>" + "</html>";
     ...
     *           "onResume"); <br />} </code>
     */
    public WebView createWebViewInApp(String webContent, int fontSizeDefault,
            String backgroundColor, String textColorDefault)
    {
        return createWebViewInApp(webContent, fontSizeDefault, backgroundColor,
                textColorDefault, null);
    }
            ...
            String fontAssetPath)
    {
        ...
        {
            contentShown = String.format(Locale.getDefault(), WEB_CONTENT,
                    fontSizeDefault, bgColor + textColor, webContent);
        }
        else
        {
            contentShown = String.format(Locale.getDefault(),
                    WEB_CONTENT_FONT_FACE_APPLIED, fontAssetPath,
                    fontSizeDefault, bgColor + textColor, webContent);
        }
