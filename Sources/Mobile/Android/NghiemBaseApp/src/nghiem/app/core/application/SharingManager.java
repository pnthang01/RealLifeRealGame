...
public class SharingManager extends BaseController
{
    ...
    public void shareOnFacebook(String caption, String text,
            String facebookShareLink)
    {
        ...
        if (!facebookAppFound)
        {
            String sharerUrl = mContext.getString(R.string.facebook_link)
                    + "?u=" + facebookShareLink;
    ...
    public void shareOnTwitter(String message)
    {
        String tweetUrl = String.format(
                mContext.getString(R.string.twitter_link) + "?text=%s", message);
    ...
    {
        List<ResolveInfo> matches = mContext.getPackageManager().queryIntentActivities(
                intent, 0);
        for (ResolveInfo info : matches)
        {
            if (info.activityInfo.packageName.toLowerCase(Locale.getDefault()).startsWith(
                    packageName))
            {
