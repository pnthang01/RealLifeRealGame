...
public class ImageManager extends BaseController
{
	...
	public boolean loadDrawable(View imageView, String link, int viewWidth, int viewHeight, boolean useBackgroundTransitionAnimation)
	{
		...
		if (!TextUtils.isEmpty(colorString))
		{
			...
			LOG.debug("Load drawble from link --- Image Color = " + colorString + " => " + hexString);
			try
			{
	...
	public void saveToSdcard(String link, Bitmap bitmap, boolean useHighQuality)
	{
		...
		if (bitmap == null)
		{
			LOG.error("Save image to SDCard: cancel because bitmap is null (link: " + link + ")");
			return;
		}
		...
		{
			LOG.debug("Save image to SDCard --- cancel - File is exists! " + link);
	...
	public void deleleFromSdcard(String linkWithColor)
	{
		...
		if (linkWithColor.contains(COLOR_PREFIX))
		{
			...
			{
				// At least link = http://
				LOG.error("Delete image from SDCard: cancel - wrong link: " + link);
				return;
			}
		}
		else
		...
		{
			LOG.error("Delete image from SDCard: link is not cached yet!!! (link: " + link + ")");
		}
	}
