...
public class ImageManager extends BaseController
{
    private static final int MAX_CACHE_SIZE = 5 * 1024 * 1024;

    private static final LruCache<String, Bitmap> sBitmapCacher = new LruCache<String, Bitmap>(
            MAX_CACHE_SIZE);
     ...
     */
    public boolean loadDrawable(View imageView, String link, int viewWidth,
            int viewHeight, boolean useBackgroundTransitionAnimation)
    {
        if (TextUtils.isEmpty(link))
        {
            LOG.error("Load drawble from link --- null image URL!!!");
            return true;
        }
        ...
        if (bitmap != null)
        {
            applyDrawableToViewWithAnimation(imageView, bitmap,
                    useBackgroundTransitionAnimation);
            LOG.debug("Loaded bitmap from cach with id = " + link);
            return true;
        }
        ...
        if (!TextUtils.isEmpty(colorString))
        {
            ...
            LOG.debug("Load drawble from link --- Image Color = " + colorString
                    + " => " + hexString);
            try
            {
            ...
            {
                ((ImageView) imageView).setImageDrawable(new ColorDrawable(
                        color));
            }
            else
            {
        ...
        {
            // At least link = http://
            LOG.error("Load drawble from link --- Wrong URL!!! URL = " + link);
            return true;
        }
    ...
    public void applyDrawableToViewWithAnimation(View view, Bitmap bitmap,
            boolean useBackgroundTransitionAnimation)
    {
        ...
        {
            applyDrawableToView(view, new BitmapDrawable(view.getResources(),
                    bitmap));
            return;
        }
    ...
    public void saveToSdcard(String link, Bitmap bitmap, boolean useHighQuality)
    {
        if (TextUtils.isEmpty(link))
        {
            LOG.error("Save image to SDCard: cancel because URL is null");
            return;
        }
        if (bitmap == null)
        {
            LOG.error("Save image to SDCard: cancel because bitmap is null (link: "
                    + link + ")");
            return;
        }
        if (!DeviceManager.isExternalStorageSupportWrite())
        {
            LOG.error("Save image to SDCard: cancel, there is no SDCard or it is read only");
            return;
        }
        ...
        if (!file.exists())
        {
            ...
            {
                LOG.error("Save image to SDCard: error =  " + e.getMessage());
                return;
            }
        }
        else
        {
            LOG.debug("Save image to SDCard --- cancel - File is exists! "
                    + link);
            LOG.debug("Save image to SDCard --- delete local file");
            file.delete();
        }

        LOG.debug("Save image to SDCard --- Save with new file");
        ...
        } finally
        {
            if (fileOutputStream != null)
            {
                ...
                {
                    LOG.error("Save image to SDCard: error =  "
                            + e.getMessage());
                }
            }
        }
        LOG.debug("Saved image to SDCard Successfully! (link: " + link + ")");
    }
    ...
    public void deleleFromSdcard(String linkWithColor)
    {
        ...
        {
            LOG.error("Delete image from SDCard: cancel - Null link");
            return;
        }
        if (!DeviceManager.isExternalStorageSupportWrite())
        {
            LOG.error("Delete image from SDCard: cancel - There is no SDCard or it is read only");
            return;
        }
        ...
        {
            link = linkWithColor.substring(0,
                    linkWithColor.indexOf(COLOR_PREFIX));
            if (link == null || link.length() < 8)
            {
                // At least link = http://
                LOG.error("Delete image from SDCard: cancel - wrong link: "
                        + link);
                return;
            }
        }
        ...
        {
            file.delete();
            LOG.debug("Delete image from SDCard Successfully!!! (link: " + link
                    + ")");
        }
        else
        {
            LOG.error("Delete image from SDCard: link is not cached yet!!! (link: "
                    + link + ")");
        }
    }
    ...
    public Bitmap loadFromSdcard(String link, int viewWidth, int viewHeight)
    {
        if (TextUtils.isEmpty(link))
        {
            LOG.error("Load image from SDCard: cancel - Null link");
            return null;
        }
        if (!DeviceManager.isExternalStorageSupportWrite())
        {
            LOG.error("Load image from SDCard: cancel - There is no SDCard or it is read only");
            return null;
        }
        ...
        if (!file.exists())
        {
            LOG.error("Load image from SDCard: cancel - This was not saved to SDCard yet");
            return null;
        }
    ...
    private String getFileName(String link)
    {
        ...
        return linkParts[len - 3] + "_" + linkParts[len - 2] + "_"
                + linkParts[len - 1];
    }
