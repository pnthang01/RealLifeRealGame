package nghiem.app.core.animation;
...
public class SlideAnimation extends Animation implements AnimationListener
{
    ...
    public SlideAnimation(View view, int min, int max, int reapeat,
            boolean isVertical, boolean isExpand)
    {
        mView = view;
    ...
    public void onAnimationRepeat(Animation anim)
    {
        if (mIsVertical)
        {
            mParams.height += mIsExpanded ? (mMin - mMax) / mRepeat : (mMax - mMin)
                    / mRepeat;
        }
        else
        {
            mParams.width += mIsExpanded ? (mMin - mMax) / mRepeat : (mMax - mMin)
                    / mRepeat;
        }
        mView.setLayoutParams(mParams);
    }
