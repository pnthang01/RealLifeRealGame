...
{
    private static final Logger LOG = new Logger(StorageUtils.class);
                ...
                "/proc/mounts")))
        {
            ...
            while ((line = reader.readLine()) != null)
            {
                ...
                if (line.contains("vfat") || line.contains("/mnt"))
                {
                    ...
                    {
                        paths.add(path);
                        list.add(0, new StorageInfo(path, isInternal, readonly,
                                -1));
                    }
                    else if (line.contains("/dev/block/vold"))
                    {
                                ...
                                && !line.contains("tmpfs"))
                        {
                            ...
                            list.add(new StorageInfo(mountPoint, false,
                                    readonly, currentDisplayNumber++));
                        }
                    }
                }
            }
    ...
    public static void debugStorages()
    {
        if (BuildConfig.DEBUG)
        {
            ...
            {
                LOG.debug(storageInfo.getDisplayName());
            }
        }
    }
     ...
     */
    public static String getContentFromBuffer(ByteArrayOutputStream buffer,
            String charset)
    {
        String content = null;
        try
