...
public class RlrgApp extends NghiemBaseApp
{
    ...
    public void init()
    {
        ...
        badges = (Badges) gson.fromJson(preferences.loadJsonBadges(),
                Badges.class);
        categories = (Categories) gson.fromJson(
                preferences.loadJsonCategories(), Categories.class);
        tasks = (Tasks) gson.fromJson(preferences.loadJsonTasks(), Tasks.class);
        achievements = (Achievements) gson.fromJson(
                preferences.loadJsonAchievements(), Achievements.class);
    }
    ...
    public void checkAchievemnt()
    {
        for (Badge badge : badges.getData().getElements())
        {
            ...
            {
                if (eligibility[0].equals(category.getName())
                        && Integer.parseInt(eligibility[1]) == tasks.getTaskCount(category))
                {
                    List<Achievement> achievementList = achievements.getData().getElements();
                    Achievement achievement = new Achievement();
                    achievement.setId(achievementList.get(
                            achievementList.size() - 1).getId());
                    achievement.setBadge(badge);
                    achievement.setAchievedTime(DateTime.now().getMillis());
                    achievementList.add(achievement);
                    DataPreferencesManager.getInstance().saveJsonAchievements(
                            new Gson().toJson(achievements, Achievements.class));
                    break;
                }
            }
        }
