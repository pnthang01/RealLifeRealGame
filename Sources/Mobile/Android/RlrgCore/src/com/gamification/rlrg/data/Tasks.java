...
public class Tasks extends BaseEntity<Tasks.TaskList>
{
    ...
    private long getTimeFromToday(int days)
    {
        ...
        return now.toLocalDate().plusDays(days).toDateTimeAtStartOfDay(
                now.getZone()).getMillis();
    }
    ...
    public List<Task> getNormalTasks()
    {
        ...
        for (Task task : getData().getElements())
        {
            ...
            if (!category.equals("Login") && !category.equals("Create")
                    && !category.equals("Finish"))
            {
                result.add(task);
            }
        }
    ...
    public void addTask(String category, long completeTime,
            String difficultyLevel, String name, String point, String status)
    {
        String id = String.valueOf(getTaskLastId() + 1);
        Task task = new Task();
        task.setCategory(RlrgApp.getInstance().getCategories().getCatgory(
                category));
        if (completeTime != 0)
        {
        ...
        DataPreferencesManager.getInstance().saveJsonTasks(
