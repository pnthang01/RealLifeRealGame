...
import com.rlrg.webserver.admin.service.MvcConfigurationService;

@RequestMapping("/config")
@Controller
public class MvcConfigurationController {
	private static final Logger LOG = LoggerFactory.getLogger(MvcConfigurationController.class);
	
	@Autowired
	private MvcConfigurationService configService;
	...
	public String manage(@RequestParam(value = "page", required = false, defaultValue = "1") Integer page,
		...
		try {
			ResultList<ConfigDTO> result = null;
			if (null == searchForm || null == searchForm.getKeyword()) {
				model.addAttribute("searchForm", new SearchForm());
				result = configService.getAllConfigurations(page);
			} else if (bResult.hasErrors()) {
				return "config.manage";
			} else {
				result = configService.searchConfigByKeyword(searchForm.getKeyword(), page);
				model.addAttribute("searchForm", searchForm);
			} 		
			//
			if (null != result) {
				model.addAttribute("totalPage",Math.ceil((double) result.getTotal()/ Constants.PAGE_SIZE));
				model.addAttribute("configs", result.getList());
			}
			return "config.manage";
