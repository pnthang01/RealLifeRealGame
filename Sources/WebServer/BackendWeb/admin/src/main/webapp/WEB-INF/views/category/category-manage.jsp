...
<div class="content-box column-right main">
	...
	<div class="content-box-content">
		<div style="text-align:right">
			<form:form method="GET" commandName="searchForm" action="/category/manage">
				<table>
					<tr>
					...
					<td colspan="4">
						...
						<div class="pagination">
							<c:forEach begin="1" end="${totalPage}" var="count">
								...
								<c:if test="${count == 1}">
									<a href="<c:url value="/category/manage${url}"/>">First</a>
								</c:if>
								<c:choose>
									<c:when test="${count == param.page}">
										<a href="<c:url value="/category/manage${url}"/>"
											class="graybutton pagelink active"><c:out
												value="${count}" /></a>
									</c:when>
									<c:otherwise>
										<a href="<c:url value="/category/manage${url}"/>"
											class="graybutton pagelink"><c:out value="${count}" /></a>
									</c:otherwise>
								</c:choose>
								<c:if test="${count == totalPage}">
									<a href="<c:url value="/category/manage${url}"/>">Last</a>
								</c:if>
							</c:forEach>
						</div>
					</td>
					...
					<c:otherwise>
						<c:forEach var="category" items="${categories}" begin="0"
							...
							<tr>
								...
								<td style="width: 45%"><c:out
										value="${category.description}">No description</c:out></td>
