...
<%-- 							<a href="<c:url value="/achievement/add.html"/>" class="graybutton">Add new achievement</a> --%>
						</div>
						<div class="pagination">
						<c:if test="${not empty totalPage || not empty achievements}">
							<c:url value="" var="url">
							...
							<c:forEach begin="1" end="${totalPage}" var="count">
								...
								<c:if test="${count == 1}">
									<a href="<c:url value="/achievement/manage.html${final_url}"/>">First</a>
								</c:if>
								<c:choose>
									<c:when test="${count == param.page}">
										<a href="<c:url value="/achievement/manage.html${final_url}"/>" class="graybutton pagelink active">
											<c:out value="${count}" />
										</a>
									</c:when>
									<c:otherwise>
										<a href="<c:url value="/achievement/manage.html${final_url}"/>" class="graybutton pagelink">
											<c:out value="${count}" />
										</a>
									</c:otherwise>
								</c:choose>
								<c:if test="${count == totalPage}">
									<a href="<c:url value="/achievement/manage.html${final_url}"/>">Last</a>
								</c:if>
							</c:forEach>
						</c:if>
						</div>
			...
			<tbody>
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="achievement" items="${achievements}" begin="0" end="${fn:length(achievements)}">
							<tr>
								<td><input type="checkbox" /></td>
								<td><c:out value="${achievement.id}">No id</c:out></td>
								<td><c:out value="${achievement.username}">No user name</c:out></td>
								<td>
									<a href="<c:url value="/achievement/edit.html"><c:param name="code" value="${achievement.id}"/></c:url>">
